...
class TriviaShort:
    ...
    def generate_video(self):
        ...
        song_clip = volumex(AudioFileClip(self.music).set_duration(
            i_duration * n_questions + intro_duration + outro_duration), 0.7)
        ...
        background_clip = resize(
            (
                background_clip.cutout(0, randint(1, round(background_duration) - (i_duration * n_questions))).set_duration(
                    i_duration * n_questions + intro_duration + outro_duration).set_position('center', 'center')
            ),
            height=1920,
        ).set_audio(song_clip)
